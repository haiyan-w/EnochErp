...
@interface PopMenu()<UIGestureRecognizerDelegate,UITableViewDelegate,UITableViewDataSource>
...
@property (nonatomic, readwrite, copy)NSArray * menuStrings;
...
- (instancetype)initWithArrow:(CGPoint)point menuSize:(CGSize)size arrowStyle:(CWPopMenuArrowPosition)position menuStrings:(NSArray *)strings
{
...
- (void)setUpMenuFrame
{
    ...
    self.menuView.frame = CGRectMake(_origin.x, _origin.y, _menuSize.width, _menuSize.height-_arrowHeight);
}
...
{
    PopMenuCell *cell = [PopMenuCell cellWithTableView:tableView];
    cell.labText.text = self.menuStrings[indexPath.row];
...
{
    NSLog(@"点击了%@",self.menuStrings[indexPath.row]);
    [self.delegate popMenu:self didSelectAtIndex:indexPath.row];
    [self closeMenu:NO];
}

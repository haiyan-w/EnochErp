...
//视频播放
@property (strong, nonatomic) AVPlayerView *player;
...
- (void)viewDidLoad {
    ...
    UIImage *image = [UIImage imageNamed:@"capture"];
    self.backCenterX.constant = -(self.view.bounds.size.width/2/2)-image.size.width/2/2;
...
- (void)customCamera {
    ...
    //添加一个音频输入设备
    AVCaptureDeviceDiscoverySession *audioDeviceDiscoverySession=[AVCaptureDeviceDiscoverySession discoverySessionWithDeviceTypes:@[AVCaptureDeviceTypeBuiltInMicrophone] mediaType:AVMediaTypeAudio position:AVCaptureDevicePositionUnspecified];
    AVCaptureDevice *audioCaptureDevice = [[audioDeviceDiscoverySession devices] firstObject];
    //初始化输入设备
    NSError *error = nil;
...
-(void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error{
    ...
    if (self.isVideo) {
        ...
        if (!self.player) {
            self.player = [[AVPlayerView alloc] initWithFrame:self.bgView.bounds withShowInView:self.bgView url:outputFileURL];
        } else {
            if (outputFileURL) {
